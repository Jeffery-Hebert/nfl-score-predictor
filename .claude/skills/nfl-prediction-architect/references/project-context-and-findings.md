...
- **`build_drive_rolling_features.py` has an incomplete fix**: `is_finale_week` masking is computed but never applied to the EWM calculation (mask computed, `.where()` step never added to the drive-stat columns). Low priority — only affects the currently-unused Monte Carlo model — but should be fixed before that model is ever revisited.
