...
def test_matches_completed_game_count(df):
    ...
    assert len(df) == expected, f"Model table has {len(df)} rows, schedules has {expected}"
...
def test_no_leakage_columns_present(df):
    ...
    for col in df.columns:
        if any(s in col for s in forbidden_substrings) and not col.startswith(("home_pregame_", "away_pregame_")):
            pytest.fail(f"Column '{col}' looks like a non-pregame stat leaking into the model table")
