...
def fit_lgbm(train: pd.DataFrame) -> dict:
    ...
    home_model = LGBMRegressor(**params).fit(X, train["home_score"], sample_weight=w)
    away_model = LGBMRegressor(**params).fit(X, train["away_score"], sample_weight=w)
...
def main():
    ...
    save_predictions(results, "lgbm", inputs=[MODEL_TABLE])
