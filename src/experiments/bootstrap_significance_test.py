...
SPLIT_FEATURE_COLS = [f"home_{c}" for c in SPLIT_STAT_COLS] + [
    f"away_{c}" for c in SPLIT_STAT_COLS
]
...
def build_merged_table() -> pd.DataFrame:
    ...
    home = rolling[rolling["is_home"] == 1][
        ["game_id", "team", "opponent"] + SPLIT_STAT_COLS
    ]
...
def main():
    ...
    if ci_low <= 0 <= ci_high:
        print(
            "Zero falls within the CI -- this delta is NOT statistically distinguishable from noise."
        )
    else:
        print(
            "Zero falls OUTSIDE the CI -- this delta is likely a real effect, not noise."
        )
