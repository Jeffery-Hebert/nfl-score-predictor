"""
src/experiments/test_pass_rush_split_plus_cpoe.py

Tests both changes together, correctly this time: blended EPA is REPLACED
by the pass/rush split (not added alongside it), and CPOE is added on top.
This is the fair version of the original combined test.

Run: python -m src.experiments.test_pass_rush_split_plus_cpoe
"""
...
def report(name, base_metrics, ext_metrics):
    print(f"\n{'='*60}\n{name}\n{'='*60}")
    print("BASE (blended EPA, no CPOE):")
    for k, v in base_metrics.items():
        print(f"  {k}: {v:.3f}" if isinstance(v, float) else f"  {k}: {v}")
    print("WITH SPLIT + CPOE (blend REPLACED, CPOE ADDED):")
...
def main():
    ...
    combined_cols = [c for c in FEATURE_COLS if c not in BLENDED_COLS_TO_REMOVE] + NEW_FEATURE_COLS
    ...
    for name, make_fns in [("Linear Regression", make_linear_fns), ("Poisson GLM", make_poisson_fns)]:
        ...
        ext = run_model(make_fns, df, combined_cols)
