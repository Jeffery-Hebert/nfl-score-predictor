...
FEATURE_COLS = [f"pregame_off_{c}_rate" for c in CATS] + [f"pregame_def_{c}_rate" for c in CATS] + \
               ["pregame_n_drives", "pregame_n_drives_faced"]

def main():
    rolling = pd.read_parquet("data/processed/team_drive_rolling_features.parquet")

    home = rolling[["game_id", "team"] + FEATURE_COLS].rename(columns={"team": "home_team"})
    home = home.rename(columns={c: f"home_{c}" for c in FEATURE_COLS})

    away = rolling[["game_id", "team"] + FEATURE_COLS].rename(columns={"team": "away_team"})
    ...
    final = merged.merge(
        schedules[["game_id", "home_team", "away_team", "season", "week", "gameday", "home_score", "away_score"]],
