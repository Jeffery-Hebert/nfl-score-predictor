...
STAT_COLS = ["team_score", "opp_score", "off_epa_per_play",
...
def add_pregame_rolling_features(group: pd.DataFrame, halflife_days: float) -> pd.DataFrame:
    group = group.sort_values("gameday").reset_index(drop=True)
    for col in STAT_COLS:
        ewm = group[col].ewm(halflife=pd.Timedelta(days=halflife_days),
                              times=group["gameday"]).mean()
    ...
    group["prior_games_played"] = range(len(group))  # 0 for first game, 1 for second, etc.
    return group
...
def main():
    ...
    print(f"Built rolling features for {result['team'].nunique()} teams, "
